...
static bool tegra_dvfs_core_disabled = false;
#else
...
/* Freq:     216/312  456       608  760           816/912      1000        1100        1200 */
...
#ifdef CONFIG_TEGRA_OC
	/* Cpu voltages (mV): 	   750, 775, 800, 825, 850, 875,  900,  925,  955,  975,  1000, 1025, 1050, 1100, 1125 */
/* Cpu voltages from regulator:    775  775  775  800  800  800   850   850   850   875	  950	950   950   1000  1000  1050  1050  1175*/
	CPU_DVFS("cpu", 0, 0, MHZ, 314, 314, 314, 456, 456, 456,  608,  608,  608,  760,  817,  817,  912,  1000, 1000, 1100, 1100, 1200),
	CPU_DVFS("cpu", 0, 1, MHZ, 314, 314, 314, 456, 456, 456,  618,  618,  618,  770,  827,  827,  922,  1000, 1000, 1100, 1100, 1200),
	CPU_DVFS("cpu", 0, 2, MHZ, 494, 494, 494, 675, 675, 817,  817,  922,  922,  1000, 1000, 1100, 1100, 1200),
	CPU_DVFS("cpu", 0, 3, MHZ, 730, 760, 845, 845, 940, 1000, 1000, 1100, 1100, 1200),
#else
...
#ifdef CONFIG_TEGRA_OC
	CPU_DVFS("cpu", 1, 0, MHZ, 380, 380, 503, 503, 655, 655,  798,  798,  902,  902,  960,  1000, 1000, 1100, 1100, 1200),
	CPU_DVFS("cpu", 1, 1, MHZ, 389, 389, 503, 503, 655, 760,  798,  798,  950,  950,  1000, 1000, 1100, 1100, 1200),
	CPU_DVFS("cpu", 1, 2, MHZ, 598, 598, 750, 750, 893, 893,  1000, 1000, 1100, 1100, 1200),
	CPU_DVFS("cpu", 1, 3, MHZ, 730, 760, 845, 845, 940, 1000, 1000, 1100, 1100, 1200),
#else
	...
	CORE_DVFS("emc",     -1, 1, KHZ, 47500,  126668, 380000, 380000, 380000, 380000, 760000),
#else
	/* Core voltages (mV):           950,    1000,   1100,   1200,   1225,   1275,   1300 */
...
#ifdef CONFIG_TEGRA_OC
	CORE_DVFS("2d",      -1, 1, KHZ, 133000, 171000, 247000, 333500, 333500, 361000, 361000),
	CORE_DVFS("epp",     -1, 1, KHZ, 133000, 171000, 247000, 333500, 333500, 361000, 361000),
#else
...
#ifdef CONFIG_TEGRA_OC
	CORE_DVFS("vde",      1, 1, KHZ, 123500, 152000, 237500, 350000, 350000, 350000, 350000),
#else
